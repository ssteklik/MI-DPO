...
public class RealisticMovementImp implements MovementStrategy {
    ...
    private static final int SLOW_VALUE = 10;

    @Override
    public Coordinates move(int g, Missile m) {
        int x = (int) (m.getBeginX() + m.getVelocity() * (m.getTime() / SLOW_VALUE) * cos(m.getAngle()));
        int y = (int) (m.getBeginY() - (m.getVelocity() * (m.getTime() / SLOW_VALUE) * cos(m.getAngle())) +
